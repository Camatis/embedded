...
def cleanup_hardware():
    ...
    global sorting_active, sorting_paused, hopper_active, conveyor_pwm, tof_sensors
...
# ==========================================
# VL6180 ToF Sensor Setup (I2C)
# ==========================================
print("\n🔍 Initializing VL6180 ToF Sensors...")

# Distance Thresholds (in mm) - TUNE THESE BASED ON YOUR SETUP
TRIGGER_THRESHOLD = 60        # Mango detected if distance < 60mm
SMALL_MAX = 60                # Small mango: < 60mm
MEDIUM_MAX = 80               # Medium mango: 60-80mm
LARGE_MIN = 80                # Large mango: > 80mm
SENSOR_READ_TIMEOUT = 2000    # Max wait time for sensor reading (ms)

tof_sensors = {}

try:
    # Initialize I2C for ToF sensors (shares same bus as PCA9685)
    i2c = busio.I2C(board.SCL, board.SDA)
    
    # Initialize all three VL6180X sensors (all at default address 0x29)
    # In production, use XSHUT pins to control multiple sensors at different addresses
    try:
        tof_sensors['trigger'] = VL6180X(i2c, address=0x29)
        print("✓ Trigger ToF sensor initialized at 0x29")
    except Exception as e:
        print(f"⚠️  Trigger sensor failed: {e}")
    
    # For production: if you have multiple sensors, enable them sequentially
    # via XSHUT pins and configure different I2C addresses
    if len(tof_sensors) > 0:
        print(f"✓ {len(tof_sensors)} ToF sensor(s) ready")
    else:
        print("⚠️  No ToF sensors initialized - check I2C wiring")
        
except Exception as e:
    print(f"⚠️  ToF sensor initialization error: {e}")
    print("Troubleshooting:")
    print("  - Check I2C wiring: SDA → GPIO 2, SCL → GPIO 3")
    print("  - Enable I2C: sudo raspi-config → Interfacing → I2C")
    print("  - Install library: pip install adafruit-circuitpython-vl6180x")
    print("  - Run diagnostics: python3 vl6180_test.py")

def read_distance_sensor(sensor_name='trigger', max_retries=3):
    """
    Read distance from ToF sensor with error handling.
    Returns distance in mm, or None if read fails.
    """
    if sensor_name not in tof_sensors:
        return None
    
    for attempt in range(max_retries):
        try:
            sensor = tof_sensors[sensor_name]
            distance = sensor.range
            if distance is not None:
                return distance
        except Exception as e:
            if attempt == max_retries - 1:
                print(f"⚠️  {sensor_name} sensor read failed after {max_retries} attempts: {e}")
            time.sleep(0.01)
    
    return None

def mango_detected():
    """Check if mango is present at trigger position using ToF sensor."""
    distance = read_distance_sensor('trigger')
    if distance is None:
        return False
    return distance < TRIGGER_THRESHOLD

def classify_mango_size_tof(distance):
    """
    Classify mango size based on ToF distance reading.
    Distance-based classification (assumes mango is detected).
    
    Tuning guide:
    - Measure actual mango sizes and distance when detected
    - Update thresholds accordingly
    - For now, using generic values - adjust after first test
    """
    if distance is None:
        return "SMALL"  # Default fallback
    
    if distance < SMALL_MAX:
        return "SMALL"
...
def get_hardware_status():
    trigger_distance = read_distance_sensor('trigger')
    return jsonify({
        'counts': {
            'small': count_small,
            'medium': count_medium,
        ...
        'sensors': {
            'trigger_distance_mm': trigger_distance,
            'mango_detected': trigger_distance < TRIGGER_THRESHOLD if trigger_distance else False
        },
...
def get_sensor_diagnostics():
    """Get detailed sensor diagnostic info"""
    trigger_dist = read_distance_sensor('trigger')
    return jsonify({
        'tof_sensors': {
            'trigger': {
                'distance_mm': trigger_dist,
                'mango_detected': trigger_dist < TRIGGER_THRESHOLD if trigger_dist else False,
                'signal_quality': 'good' if trigger_dist is not None else 'error'
            }
        },
        'thresholds': {
            'trigger_threshold_mm': TRIGGER_THRESHOLD,
            'small_max_mm': SMALL_MAX,
            'medium_max_mm': MEDIUM_MAX,
            'large_min_mm': LARGE_MIN
        }
    })
...
# ==========================================
print("\n" + "="*45)
print("✅ HARDWARE SERVICE READY (VL6180 ToF)")
print("="*45)
...
def autonomous_sorting_loop():
    ...
    PHASE 1 - Detection (Belt Stopped):
      1. Mango detected at trigger sensor (ToF distance < threshold)
    ...
    PHASE 2 - Routing (Belt Running):
      ...
      If GOOD:
        ...
        - Size determined by ToF distance reading
    ...
    while True:
        try:
            ...
            if mango_detected():
                trigger_distance = read_distance_sensor('trigger')
                print(f"\n🥭 MANGO DETECTED IN CHAMBER! (Distance: {trigger_distance}mm)")
                ...
                if is_defective:
                    ...
                    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                    # ROUTE: GOOD (Size Classification via ToF)
                    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                    print(f"🧠 GOOD mango → Classifying size using ToF...")
                    
                    # Read ToF distance to determine size
                    # Take a few readings and use most common one
                    distance_readings = []
                    end_time = time.time() + 0.5  # Quick 0.5s reading period
                    
                    while time.time() < end_time:
                        dist = read_distance_sensor('trigger')
                        if dist is not None:
                            distance_readings.append(dist)
                        time.sleep(0.05)
                    
                    if distance_readings:
                        avg_distance = sum(distance_readings) / len(distance_readings)
                        detected_size = classify_mango_size_tof(int(avg_distance))
                        print(f"📏 ToF readings (n={len(distance_readings)}): avg={avg_distance:.1f}mm → {detected_size}")
                    else:
                        print(f"📏 ToF read failed, defaulting to SMALL")
                        detected_size = "SMALL"
                ...
                clear_timeout = time.time() + 10  # 10 second timeout
                while mango_detected() and time.time() < clear_timeout:
                    time.sleep(0.05) 
